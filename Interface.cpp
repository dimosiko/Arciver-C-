	...
	else
	{
		*ARC = arc;
		...
		else
		{
			if (pos > 3)
			{
				roles[sizea] = pos; //ôàéë ïîä ëèñòèíã/ïðîâåðêó
			}
			else
			{
				roles[sizea] = pos + 5; //ôàéë àðõèâà
			}
...
int arcCheck(char *mode, char *file, FILE **ARC)
{
	...
	if (!_access(file, 0))
	{
		if (mode == "rb")
		{
			...
			{
				printf("Àðõèâ íå ìîæåò áûòü îòêðûò äëÿ ÷òåíèÿ. Äåéñòâèÿ ñ àðõèâîì %s ïðåêðàùåíû\n", file);
				return 1;
			}
		}
		else
		{
			...
			{
				printf("Àðõèâ íå ìîæåò áûòü îòêðûò äëÿ ÷òåíèÿ è çàïèñè. Äåéñòâèÿ ñ àðõèâîì %s ïðåêðàùåíû\n", file);
				return 1;
			}
		}
	}
	...
	if (!arc)
	{
		if (!_access(file, 0))
		{
			printf("Ôàéë àðõèâà %s íåäîñòóïåí äëÿ îòêðûòèÿ", file);
		}
		else
		{
			while (choise == 0)
			{
				printf("Ôàéë àðõèâà %s íå ñóùåñòâóåò. Ñîçäàòü åãî? y/n\n", file);
				fflush(stdin);
				scanf("%c", &choise);
				...
				if ((choise == 'y') || (choise == 'Y'))
				{
					...
					if (!arc)
					{
						printf("Íå óäàåòñÿ ñîçäàòü ôàéë àðõèâà %s. Äåéñòâèÿ ñ íèì ïðåêðàùåíû\n", file);
						*ARC = arc;
						return 1;
					}
					else
				...
				{
					printf("Äåéñòâèÿ ñ àðõèâîì %s ïðåêðàùåíû\n", file);
					*ARC = arc;
					return 1;
				}
				else
				{
					printf("Êîìàíäà íå ðàñïîçíàíà\n");
					choise = 0;
				}
			}
		}
