...
int Listing(FILE *arc)
{
	...
	printf("Èìÿ                 | Ðàçìåð ôàéëà           | Ðàçìåð â àðõèâå        | Ïðèíöèï ñæàòèÿ | Äàòà ïîñë. èçìåíåíèÿ ôàéëà\n"); //20 1 24 1 24 1 16 1
	...
	while ((len == BUF_SIZE) || (k != len))
	{
		...
		{
			printf("Ôàéë íå ÿâëÿåòñÿ ïîääåðæèâàåìûì ôàéëîì àðõèâà èëè èñïîð÷åí\n");
			return 1;
		}
		k += 10;
		mode = (rbuf + k)[0];
...
int Listing(FILE *arc)
{
	...
	return 0;
		...
		if (IfCorrupted(arc, k, size, rbuf, &len, &k))
		{
			printf("×òåíèå äàëåå íåâîçìîæíî. Àðõèâ èñïîð÷åí");
			return 1;
		}
		else
		{
